    ...
    def calculate_stat_change(self, stat, stat_value):
        for status in self.Statuses:
            if supported_Statuses[status]['apply_type'] == stat:
                ...
                stat_value -= player.Statuses[status]['amount']
        for armor_piece in player.Inventory.current_equips:

            if player.Inventory.current_equips[armor_piece].change_type == stat: # last
                pass
    ...
    class Inventory:
        ...
        def throw_away(item, enemy):
            ...
            if item in player.Inventory.items:
                ...
                if not isinstance(player.Inventory.current_equips[item], Bare):
                    if item == "head":
                        ...
                        if case_inventory is None:
                            ...
                            while True:
                                ...
                                def handle_slot(slot: str, joke_text: str = ''):
                                    ...
                                    if decision == "Unequip":
                                        ...
                                        player.Inventory.throw_away(slot, enemy)
                            ...
                            try:
                                ...
                                if item_to_inspect is not None:
                                    description = raw_inventory_items[inventory_items.index(item_to_inspect)].inspect()
                                else:
...
def game_setup():
    ...
    # Creating the necessary folders in the game directory if they have been deleted
    if not os.path.exists(project_path + "Saves"):
        os.makedirs(project_path + "Saves")

    if not os.path.exists(os.path.dirname(project_path + "Player saves")):
        os.makedirs(project_path + "Player saves")

    if not os.path.exists(os.path.dirname(project_path + "General info")):
        os.makedirs(project_path + "General info")
    ...
    try:
        ...
        try:
            ...
            quickedit = 0
            print("You are probably not using windows. Shit might not work and crash")
    ...
    input()
        ...
        # IF (read registry) they are wrong, edit resitry values, then restart

    if 1:
        # Disabling quickedit and enabling legacy console for a better experience
        path = r"Console"
    ...
    if version in accepted_windows_versions:
        ...
        print("Unsupported OS. Contact the game developer at matistjati@outlook.com for help")
